...
Download the directory and [install it in Chrome as an unpacked extension](https://developer.chrome.com/extensions/getstarted#unpacked). An icon should appear in your browser extensions list. Just click on it to enable Requests-Audio, click a tab with a website and try refreshing the website to trigger all of the web requests needed to serve up that page. You should start hearing sounds!
...
With the extension running, it's helpful to have the Developer Tools open in Chrome and look at the Network tab to see the web requests happen in real time while listening to the audio.

I recorded audio of various websites which highlight different patterns...
